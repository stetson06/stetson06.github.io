...
For both OpenAI [publication](https://platform.openai.com/docs/overview) and LangChain, we first log-in (with creation of these accounts preceding as necessary) and create the necessary API keys for each. These keys are then loaded onto an .env (Notepad) text file that is read by and enables Python to connect to LangChain. A yaml file
