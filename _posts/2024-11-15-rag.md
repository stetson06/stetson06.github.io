...
With the virtual environment established, we proceed to coding on Python. Within the Spyder IDE, we ensure our working directory is pointing toward the folder containing the model's files. The first few sections of the code are administrative - they set up permissions and load the info file containing 32 pairs of questions and answers from ABC Grocery's notional Help Center (in real life, this set of Q/As would be much larger).
...
The console prints out the 32 Q/A pairs to indicate the IDE has properly read the file. The last of the 32 pairs is shown below:

    ![rag_qa](/img/posts/rag_qa_32.png)
