...
Despite their extensive training, LLMs are generally unaware of proprietary information not available online. So asking an LLM for specific information (e.g., let's say personnel decisions based on specific HR policies of our fictional ABC Grocery store that is not posted on their website) that is "unknowable" places LLMs that want to fully address prompts in a difficult position. In such a predicament, LLMs will reply with either an apology that it can't find the answer or worse, it will hallucinate an inaccurate response.
...
To address this inherent weakness in prompting, a new process, RAG, has emerged that leverages a database of information and AI tools that identify which "chunks" of the database are most relevant to the user's prompt and feed just that useful information into the prompt window. This clever technique very efficiently allows an informed LLM response without violating prompt window constraints - it is the brilliance of RAG models and behind the chatbots we commonly see on business websites today.
...
Our fictional local business is the ABC Grocery Store. Their leaders have requested an AI chatbot to allow customers to ask ABC Grocery-specific questions on their company website and receive immediate, accurate responses.
